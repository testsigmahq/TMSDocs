...
metadesc: 'When a sprint starts, the Generator Agent detects it and creates test cases automatically from the sprint stories and linked artifacts. Learn how sprint detection works.'
...
contextual_links:
  ...
  - type: link
    name: 'What the Generator Agent Uses as Context'
    url: '#what-the-generator-agent-uses-as-context'
  - type: link
...
**When a sprint starts, the Generator Agent detects it and generates test cases automatically.**

---

In Sprints, test case generation is triggered by your sprint workflow, not manually. When a sprint starts in your connected project management tool, the Generator Agent detects the new sprint and notifies you. You confirm, and it pulls context from the sprint stories and linked artifacts to generate test cases.
...
When a sprint starts in your connected project management tool, the Generator Agent automatically detects it and shows a notification.

1. From the left navigation bar, go to **QI Home**.

2. Select the **Sprints** tab.

3. When the Generator Agent detects a new sprint, a notification appears: **A new sprint is live. Start creating test cases for it?** The notification shows the sprint name and start date.

4. Click **Start Generating Tests** to begin. The Generator Agent moves the sprint to **In Progress** and begins generating test cases.
...
## **What the Generator Agent Uses as Context**

The Generator Agent automatically pulls context from the sprint stories and their linked artifacts. You do not need to attach anything manually.

Context the Generator Agent uses includes:
...
[[info | **NOTE**:]]
| The Generator Agent can only read artifacts that are either publicly accessible or connected as integrations. If a Confluence page or Figma file is private and the integration is not set up, it will not be able to use it as context.

---

## **Generate Test Cases for an Unprocessed Sprint**

If you clicked **Not Now** on a sprint or if the Generator Agent missed a sprint notification, you can trigger generation manually.
...
3. Click **Generate Test Cases** next to the sprint name. The Generator Agent begins generating test cases using the same process as automatic detection.

---

## **What Happens During Generation**

The Generator Agent uses the same generation process as Ad-Hoc. You can watch the progress in the left panel as each step completes. Generation typically takes one to two minutes.

<!-- screenshot: Generation progress log in left panel showing steps completing -->

See [Generate Test Cases](https://testsigma.com/docs/arcus/qi-home/ad-hoc/generate-test-cases/) in the Ad-Hoc section for details on what the Generator Agent does during generation and how to read the generation summary.
