...
Both generate test cases using the Generator Agent. The difference is in how generation is triggered and where the context comes from. In Ad-Hoc, you manually attach context sources and click Generate. In Sprints, the Generator Agent detects when a sprint starts in your connected project management tool and offers to generate automatically using the sprint stories and linked artifacts. Everything after generation — reviewing test cases, coverage, test plans, and quality metrics — is the same for both. See the [Overview](https://testsigma.com/docs/arcus/qi-home/overview/).

### **2. Why did generation skip some scenarios or leave scenario outlines empty?**

The Generator Agent creates full test cases for the scenarios it can complete based on your context. For areas where the context was incomplete, or where the Coverage Agent flagged that coverage was needed but the scenario could not be fully expanded, the Generator Agent creates Remaining Scenario Outlines instead. These are stubs that mark areas needing coverage. Open the Playground and describe the missing scenarios to fill them. See [Use Playground](https://testsigma.com/docs/arcus/qi-home/use-playground/).

### **3. I clicked Not Now on a sprint notification. Can I still generate test cases?**

Yes. Go to **QI Home > Sprints** tab. Find the sprint with **To Do** status and click **Generate Test Cases** next to it. See [Generate Test Cases](https://testsigma.com/docs/arcus/qi-home/sprints/generate-test-cases/).
