...
metadesc: 'The controls available to you during an Agentic Learning session — panel controls, session states, providing input when the Runner Agent gets stuck, and re-learning a test case.'
...
contextual_links:
  ...
  - type: link
    ...
    url: '#session-states'
  - type: link
    name: 'Providing Input When the Runner Agent Gets Stuck'
    url: '#providing-input-when-the-runner-agent-gets-stuck'
  - type: link
...
| :--- | :--- |
| **Start Learning Live App** | Start the learning session. The Runner Agent begins walking through the test case steps on the live application. This button appears when the session is ready but has not yet started. |
| **Record Step** | Manually records the current state of the browser or device as a step. Use this if the Runner Agent misses a step or if you want to add a step that is not in the original test case. |
| **Pause** | Pauses the Runner Agent's execution. It stops walking through steps and waits for you to resume. |
...
| :--- | :--- | :--- |
| **Learning** | The Runner Agent is actively walking through steps. | Steps list with spinner on the current step. "Learning..." status in the panel. |
| **Atto needs user's input** | The Runner Agent has reached a step it cannot complete without guidance. The session is paused. | Yellow banner on the test case view: "Atto is waiting on your input to continue learning." **Go to Session** button. |
| **Completed** | The Runner Agent has walked through all steps. | Steps list with checkmarks. "Generate Automated Steps", "Save to Library", and "Reject" buttons appear. |
| **Generation Failed** | The Runner Agent could not generate automated steps from the learned session. | "Generation Failed" error with an **Edit** option. |

---

## **Providing Input When the Runner Agent Gets Stuck**

When the Runner Agent needs your input, a banner appears on the test case view.
...
2. In the browser, complete the action the Runner Agent could not perform. For example, if it stopped at a CAPTCHA or a multi-factor authentication step, complete it manually.

3. After completing the action, the Runner Agent resumes walking through the remaining steps automatically.
