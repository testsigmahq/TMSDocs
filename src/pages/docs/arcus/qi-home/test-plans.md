...
metadesc: 'The Runner Agent automatically generates four nested test plan types for every sprint and Ad-Hoc session: Smoke, Feature, Regression, and Deep Regression. Learn how to review and run them.'
description: 'Learn how the Runner Agent auto-generates the four test plan types, and how to review, accept, and link them.'
noindex: false
order: 13.28
...
**The Runner Agent automatically generates test plans for every sprint and Ad-Hoc session. Review them, understand their scope, and run them when you are ready.**

---

The Runner Agent generates four types of test plans as soon as test cases exist in your session. You do not create or configure them manually. Each plan type covers a different scope of your test cases, and they are always generated as a nested set, meaning the narrowest plan is always a subset of the next broader one.
...
> - Test cases have been generated and accepted for the session. See [Review and Accept Test Cases](https://testsigma.com/docs/arcus/qi-home/review-and-accept-test-cases/).
...
The Runner Agent generates these four test plan types automatically. Each is a superset of the one before it.
...
[[info | **NOTE**:]]
| The topmost plan type that the Runner Agent generates defines the scope used to calculate the Confidence metric. The topmost plan type you actually execute provides the pass rate results. If the Runner Agent generates all four types but you only run Smoke, Confidence reflects that the narrowest coverage was validated. See [Quality Intelligence Metrics](https://testsigma.com/docs/arcus/qi-home/quality-intelligence-metrics/).
...
The Runner Agent detects the application type from your test cases and suggests a machine configuration for each test plan. The detected types include Web, Android, and iOS.

[[info | **NOTE**:]]
| Machine configuration detection uses the test case steps and context to determine the application type. If your session includes test cases for multiple application types, the Runner Agent may suggest separate configurations for each plan.
...
When you accept, the Runner Agent creates the following in your project:
...
4. The Runner Agent creates all four plans and their test runs in your project.
...
After accepting the test plans, you can link each plan type to an existing test plan in Testsigma. Linking connects the Runner Agent's generated plan to your automation setup so that automated run results flow back into QI Home.
...
Each test run represents a set of test cases grouped by feature area or module within a plan. When you accept the test plans, the Runner Agent creates one test run per feature area across all four plan types. These runs appear both inside their respective plan and in the global Test Runs list.
