...
contextual_links:
  ...
  - type: link
    name: 'What Gets Generated After Mapping'
    url: '#what-gets-generated-after-mapping'
  - type: link
...
Developer Context Mapping connects the tools your developers already use to the Quality Intelligence loop in Arcus. When a developer opens a pull request or finishes working in a CLI tool, the Coverage Agent captures that context and surfaces it in QI Home for test generation. Developers keep their existing workflow. QA reviews what arrives and maps it to the right sprint or Ad-Hoc session.
...
Developer Context Mapping treats the coding session as the record of what changed. The Coverage Agent captures the session and identifies changed areas with no corresponding tests, the Generator Agent creates test cases from it for you to review, and QI Home reports the result through the Quality Intelligence Metrics. Developers continue working in their existing tool, and the test suite is built from the work they have already done rather than reconstructed afterwards.
...
* **Engineering leads and release managers** read the Coverage, Release Readiness, and Release Gate values that result. See [Quality Intelligence Metrics](https://testsigma.com/docs/arcus/qi-home/quality-intelligence-metrics/).
...
All context lands here first. The Coverage Agent shows an AI-generated suggestion for which sprint and story the context belongs to. You review the suggestion and map it manually, or dismiss it if it is not relevant.

**Sprint**

Context mapped to a sprint contributes to test generation for that sprint. The Generator Agent creates new test cases or updates existing ones based on the context content. The sprint's coverage, pass rate, and release readiness metrics update accordingly.
...
## **What Gets Generated After Mapping**

Mapping is the trigger. Once context is assigned to a sprint or Ad-Hoc session:

* The **Generator Agent** creates **test cases** based on the pull request diff or captured context. You review each case and accept, edit, or reject it before it is added to the sprint.
* The **Coverage Agent** identifies **coverage gaps**, areas of the change with no corresponding tests.
* The **Runner Agent** organizes **a test plan** into Smoke, Feature, Regression, and Deep Regression suites.

Execution results from these test plans feed the Quality Intelligence Metrics. See [Quality Intelligence Metrics](https://testsigma.com/docs/arcus/qi-home/quality-intelligence-metrics/) for how Coverage, Pass Rate, Confidence, Release Readiness, and Release Gate are calculated.

---

## **When CLI Context and a Pull Request Overlap**

Developers using a CLI tool often raise a pull request for the same work they captured as context. The Coverage Agent handles this automatically:

* **Context still unmapped when the PR is raised** — The Coverage Agent converts it into a GitHub context. The CLI context and the pull request are unified as a single entry in Unmapped Context. Its prompt history and code changes are analyzed together with the PR diff.
...
|------|-----------------|-------|
| GitHub | Pull requests from connected repositories | [Connect GitHub](https://testsigma.com/docs/arcus/qi-home/plugins/connect-github/) |
| Claude Code | Coding context: prompts, tool calls, file changes | [Connect Claude Code](https://testsigma.com/docs/arcus/qi-home/plugins/connect-claude-code/) |
| Cursor | Coding context: prompts, tool calls, file changes | [Connect Cursor](https://testsigma.com/docs/arcus/qi-home/plugins/connect-cursor/) |
| GitHub Copilot | Coding context: prompts, tool calls, file changes | [Connect GitHub Copilot](https://testsigma.com/docs/arcus/qi-home/plugins/connect-github-copilot/) |
| Codex | Coding context: prompts, tool calls, file changes | [Connect Codex](https://testsigma.com/docs/arcus/qi-home/plugins/connect-codex/) |
