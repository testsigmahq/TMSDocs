...
page_title: 'Connect GitHub Copilot'
metadesc: 'How to install and connect the Arcus plugin for GitHub Copilot so your coding context is captured and sent to QI Home for test generation.'
description: 'How to install and connect the Arcus plugin for GitHub Copilot so your coding context is captured and sent to QI Home for test generation.'
noindex: false
order: 13.44
page_id: 'qi-home-connect-github-copilot'
warning: false
contextual_links:
  - type: section
    name: 'Contents'
...
**Install the Arcus plugin for GitHub Copilot so that your coding context is captured and sent to QI Home for test generation.**

When the plugin is installed and connected to a project, it captures your prompts, tool calls, and file changes as you work. When you are done, you push the context to QI Home with a single command. QA maps the context to the right sprint, and the Generator Agent creates test cases from your development activity.

[[info | **NOTE**:]]
| This doc refers to the GitHub Copilot CLI plugin for Arcus, not Testsigma Copilot (the test execution feature). They are separate tools.
...
AI coding agents have made it trivial to ship large changes fast. The bottleneck has moved: it's no longer writing the code, it's trusting it. Teams using GitHub Copilot routinely merge features, refactors, and fixes faster than their QA process can keep up, and the gap between "the code is written" and "we know it works" keeps widening.
...
Arcus closes that gap by treating your GitHub Copilot session as the source of truth for what changed. It captures the session automatically, turns it into tests you review rather than write, keeps a live map of what's covered, and rolls everything up into an evidence-backed release signal.
...
1. In GitHub Copilot, run **/arcus:login**.

2. Complete the login flow in the browser. Return to GitHub Copilot when prompted.
...
The **Generator Agent** reads the captured session, code changes, the prompts you gave Copilot, the tools it ran, and drafts test cases for the behavior you actually built. You review each one and accept, edit, or reject it; accepted cases join your test suite. Nothing is auto-merged behind your back.

**2. Find gaps**

The **Coverage Agent** answers the question manual QA can never keep up with: what did we change that we haven't tested yet? It re-runs whenever context changes, a new GitHub Copilot session or a pull request, or whenever you accept a case, and surfaces untested areas with a reason for each gap.
...
* **Developers** shipping with GitHub Copilot who want their changes tested without breaking flow to write tests by hand.
...
* **Context still unmapped**: Arcus converts it into a GitHub context. The GitHub Copilot context and the pull request are unified as a single entry in Unmapped Context.
