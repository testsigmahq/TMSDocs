...
metadesc: 'Map Claude Code, Cursor, GitHub Copilot, and Codex context to sprints or Ad-Hoc sessions so the Generator Agent can create or update test cases based on development activity.'
description: 'Map Claude Code, Cursor, GitHub Copilot, and Codex context to sprints or Ad-Hoc sessions so the Generator Agent can create or update test cases based on development activity.'
noindex: false
order: 13.47
...
**Map Claude Code, Cursor, GitHub Copilot, and Codex context to sprints or Ad-Hoc sessions so the Generator Agent can create or update test cases based on development activity.**

When developers work in Claude Code, Cursor, GitHub Copilot, or Codex with the Arcus plugin installed and push context using `/arcus:push`, the Coverage Agent surfaces it in the **Unmapped Context** view in QI Home. Each context entry includes an AI-generated suggestion for which module and sprint it belongs to. Once mapped, the Generator Agent creates test cases based on the development activity captured in that context.
...
* **Tests explored** — areas the Coverage Agent identified in this context
* **Time** the context was pushed
* **AI Suggestion** — the module and sprint the Coverage Agent recommends
...
The Generator Agent triggers test generation for the selected sprint.
...
If your team has GitHub connected and a developer raises a pull request for the same work they captured in CLI context, the Coverage Agent handles the overlap automatically:

* **Context still unmapped**: The Coverage Agent converts it into a GitHub context. The CLI context and the pull request unify as a single entry. You map it as a GitHub PR, not as separate CLI context.
