...
metadesc: 'Map GitHub pull requests to sprints or Ad-Hoc sessions so the Generator Agent can create or update test cases based on code changes.'
...
**Map GitHub pull requests to sprints or Ad-Hoc sessions so the Generator Agent can create or update test cases based on code changes.**

For GitHub App setup on the developer side, see [Connect GitHub](https://testsigma.com/docs/arcus/qi-home/plugins/connect-github/).

---

When your developers open or update pull requests in GitHub, the Coverage Agent detects them and shows them in the **Unmapped Context** view in QI Home. Each pull request appears with an AI-generated suggestion for which module and sprint it belongs to. You review the suggestion and map it with a few clicks.

Once mapped, the Generator Agent creates new test cases or updates existing ones based on the code changes in that pull request.
...
- **AI Suggestion**: The module the Coverage Agent recommends mapping this PR to.
...
The Generator Agent triggers test generation for the selected sprint. New test cases are generated or existing ones are updated based on the PR's changes.
